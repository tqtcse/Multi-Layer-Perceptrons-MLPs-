...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X)
{
    ...
    xt::xarray<double> Z = xt::linalg::tensordot(X, xt::transpose(m_aWeights), 1);
...
{
    m_unSample_Counter++;
    xt::xarray<double> grad_W_batch = outer_stack(DY, m_aCached_X);
    m_aGrad_W = xt::sum(grad_W_batch, {0});
    if (m_bUse_Bias)
    {
